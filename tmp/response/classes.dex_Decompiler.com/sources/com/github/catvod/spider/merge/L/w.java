package com.github.catvod.spider.merge.L;

import com.github.catvod.spider.merge.oZP;
import java.io.IOException;

/* JADX INFO: loaded from: /tmp/decompiler/446907477a41490d9e71cfdd71d695e7/classes.dex */
public final class w extends q {
    private final boolean e;

    public w(String str, boolean z) {
        com.github.catvod.spider.merge.J.c.g(str);
        this.d = str;
        this.e = z;
    }

    @Override // com.github.catvod.spider.merge.L.s
    /* JADX INFO: renamed from: clone */
    public final Object i() {
        return (w) super.i();
    }

    @Override // com.github.catvod.spider.merge.L.q, com.github.catvod.spider.merge.L.s
    public final /* bridge */  int g() {
        return 0;
    }

    @Override // com.github.catvod.spider.merge.L.s
    public final s i() {
        return (w) super.i();
    }

    @Override // com.github.catvod.spider.merge.L.q, com.github.catvod.spider.merge.L.s
    public final s l() {
        return this;
    }

    @Override // com.github.catvod.spider.merge.L.s
    public final String s() {
        return oZP.d("560B12220034070E0328033B");
    }

    @Override // com.github.catvod.spider.merge.L.s
    public final String toString() {
        return t();
    }

    @Override // com.github.catvod.spider.merge.L.s
    final void v(Appendable appendable, int i, g gVar) throws IOException {
        Appendable appendableAppend = appendable.append(oZP.d("49"));
        boolean z = this.e;
        CharSequence charSequenceD = oZP.d("54");
        CharSequence charSequenceD2 = oZP.d("4A");
        appendableAppend.append(z ? charSequenceD : charSequenceD2).append(I());
        c cVarD = d();
        cVarD.getClass();
        int i2 = 0;
        while (true) {
            if (i2 >= cVarD.a || !cVarD.t(cVarD.b[i2])) {
                if (!(i2 < cVarD.a)) {
                    break;
                }
                a aVar = new a(cVarD.b[i2], (String) cVarD.c[i2], cVarD);
                int i3 = i2 + 1;
                String strA = aVar.a();
                String value = aVar.getValue();
                if (!strA.equals(oZP.d("560B12220034070E0328033B"))) {
                    appendable.append(' ');
                    appendable.append(strA);
                    if (!value.isEmpty()) {
                        appendable.append(oZP.d("484D"));
                        o.d(appendable, value, gVar, true, false, false, false);
                        appendable.append('\"');
                    }
                }
                i2 = i3;
            } else {
                i2++;
            }
        }
        if (!this.e) {
            charSequenceD = charSequenceD2;
        }
        appendable.append(charSequenceD).append(oZP.d("4B"));
    }

    @Override // com.github.catvod.spider.merge.L.s
    final void w(Appendable appendable, int i, g gVar) {
    }
}
